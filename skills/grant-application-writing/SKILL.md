...
description: Turn a blank funding form (Word / PDF / web-portal) plus a messy directory of an academic's materials into a filled, submission-ready application. Covers competitive research funding — grants, fellowships, awards, nominations (写本子) — across three funding modes (narrative track-record award, prospective project grant, retroactive impact funding) and many schemes (ARC DP/DECRA/Linkage/FT, CRC-P, AEA, NHMRC, ERC, NSF, Ethereum Foundation, Google/Microsoft/Amazon research awards, university internal). Use for eligibility-gate analysis, parsing a scheme into a normalized form-schema, building a reusable evidence store from source materials, drafting each field against its rubric criterion and weight, claim/verb defensibility, anti-double-counting, budget-math and compliance completeness, adversarial review, and rendering into the form's native modality (paste-ready text for portals, filled .docx, AcroForm PDF). Do not use for journal/conference papers (ml-paper-writing), citation verification only (citation-verification), rebuttal drafting (review-response), or figure rendering (paper-figure-generator).
version: 0.1.0
