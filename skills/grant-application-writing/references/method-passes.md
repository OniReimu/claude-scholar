...
- **Does:** confirm significance and innovation are *field-grounded from evidence*, not asserted. Four checks:
  - **(i) independent current literature / prior-art coverage — DEPTH is register-driven (A4).** The state of the art is set by **external comparators**, not the applicant's own survey. Absent an independent current source, reuse §1.9 primacy's `[EXTERNAL COMPARATOR NEEDED]` marker (two-mode per §1.8 — resolved or lifted to `blockers.md` at submission, never shipped raw). **How DEEP/formal the literature treatment goes depends on `classification.register`** (meeting: "research 类要 literature review；industry 类主要是故事完整性，lit-review 要求没那么强"): an **academic** scheme (ARC DP/DECRA/FT, NHMRC) wants a genuine literature review — position the work against a body of prior art, cite it, name the gap it leaves; an **industrial** scheme (ARC LP, CRC-P, DFAT) wants **story completeness** (problem → solution → benefit), where a THIN literature treatment is EXPECTED, not a defect — a couple of key comparators to establish the problem is real is enough, and a dense academic lit-review is off-genre. The (i) external-comparator requirement above still holds in BOTH (you always ground significance in something real), but the "literature-thin narrative" flag in the Out line is DOWNGRADED for `register: industrial` — do not flag an industrial application for lacking a scholarly lit-review it was never meant to carry.
...
- **Build it FROM the accumulated `scheme-kb.yaml`, not from a guess (A3).** A scheme you apply to repeatedly earns a standing, year-over-year KB (`templates/scheme-kb.template.yaml`): the **marketing / selection criteria** (the ARC term for the scored criteria) with your interpretation, and — the empirical gold — **past reviewer / panel comments** ("pings" ARC releases, feedback letters), known **failure modes**, and exemplar pointers (meeting #16/35/82: "marketing criteria 和往届 reviewer 意见都可以作为知识库逐渐累积"). When a `scheme-kb` exists, the `reviewer_model`'s scoring-emphasis + red-flags are DERIVED from it (what THIS panel actually praised/punished last round) rather than inferred from the rubric alone, and every drafted scored field is aligned to the matching `selection_criteria.interpretation`. It ACCUMULATES: after each round, append the new comments/lessons. Honesty guards: a reviewer quote is the assessor's OWN words (never invented); whether a draft ADDRESSES a past concern is a JUDGEMENT this review pass makes, not a mechanical pass — the KB informs the adversarial review, it does not auto-clear anything.
- **In:** scheme rubric + panel description + `scheme-kb.yaml` (accumulated criteria + past reviewer comments, when available). **Out:** a `reviewer_model` object + per-field framing notes.
- **Reframing the one project across panels:** **ARC** — significance + national benefit, ROPE-aware, restrained tone; **NHMRC** — health translation, RtO normalisation, structured track-record; **NSF** — intellectual merit + broader impacts as co-equal, US-centric; **ERC** — high-risk/high-gain frontier, PI-centric, ambition rewarded; **industry (Google/MS/Amazon)** — product relevance and open-source/data intent, concise; **internal (an ECR scheme)** — early-career trajectory and institutional fit over raw scale; **DFAT / bilateral / regional (NFACR Australia-China, ASEAN-Australia)** — geopolitical / national-interest alignment, delivery-partner credibility, sustainability beyond the grant.
- **Non-research-merit criteria are FIRST-CLASS, not afterthoughts.** Many non-ARC schemes (DFAT bilateral, ASEAN, government, philanthropic) score criteria that are NOT research merit: **policy / geopolitical alignment**, **foreign-interference + political / reputational risk management**, **EDI / team-diversity** (e.g. gender balance of the delivery team), **bilateral or regional benefit**, **sustainability beyond the grant's life**. These carry real weight (often ≥50% combined) and must be answered **in their own terms with the scheme's own lexicon** (`author-voice.md` §4 — mirror the rubric words: "foreign interference", "China literacy", "nation-wide benefit", "enduring outcomes"), not folded into the research case as a coda. Build the `reviewer_model` for such a panel to weight them as scored, and treat a thin answer to a 25%-weight policy criterion exactly as `criterion-readiness` (§4.4) treats any unevidenced scored criterion.
