...
对 budget-matrix (categories × year/phase × org × cash/in-kind/credit) 跑机械校验:
  ...
  8. 多年资助状态 (funding_status) —— requested (this-call) 与 indicative/conditional 分列；
                                      funding_window.funded 存在时 requested 行越窗 → FAIL
...
FAIL-CLOSED 规则 (rule FAIL → 非零退出):
  ...
  * funding_window 存在而某 requested 行含 funded 之外的年份 → FAIL (越窗，绝不静默 pass)
...
YAML schema:
    ...
    funding_window: {funded: [2026]}       # 可省; 本次 call 实际资助的年份集合
    rows:
      ...
      # 多年时行可带 funding_status: requested | indicative | conditional（缺省 requested）
