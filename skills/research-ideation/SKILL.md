...
This skill owns stages: **`intake`**, **`literature`**, **`proposal`**.
...
**Expected artifacts**:
- `literature-review.md`, `references.bib` (literature stage)
- `research-proposal.md` (proposal stage)

If no active run exists when this skill is invoked, initialize one with `initRun()` using the inferred research title.
