...
- **Full-width density** — for a `table*`, `\resizebox` may only shrink: the resulting in-table font must end up ≤ body font, never enlarged. If a two-column table is so sparse that resizebox would blow the font above body size, demote it to single-column `\resizebox{\columnwidth}` or add more metrics/columns so it genuinely needs `\textwidth`. `<!-- policy:TABLE.FULLWIDTH_FONT_DENSITY -->`
...
- **Caption says WHAT only**; push config/setup into `threeparttable` `\tablenotes`; result interpretation goes to text / takeaway box (mirrors the experiment-artifact caption convention). Deprecated as a hard check — retained as a writing guideline, enforced by `paper-self-review` at review time. `<!-- policy:FIG.SELF_CONTAINED_CAPTION -->` `<!-- policy:EXP.TAKEAWAY_BOX -->`
