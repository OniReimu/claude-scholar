...
>
> **Venue 分流**: ML 会议（NeurIPS/ICML/ICLR 等，无独立图规格）维持默认管线——字号、配色、导出格式等细节由 `scientific-figure-making` 的 `FigureStyle` 和 `finalize_figure()` 全权处理，画大缩小的口径不变。有 hard-spec 的期刊/会议（Nature 系/Science/PNAS/IEEE/ACM）先查 `references/journal-figure-specs.md`，`figsize` 直接设为目标物理尺寸、字号按 spec 表设置——此时物理尺寸口径覆盖 `FigureStyle` 的默认字号（配色与 helper 仍可用，但 rcParams 字号需按 spec 显式设定）。
>
