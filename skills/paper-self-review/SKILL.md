...
- No structural self-similarity — section-opening previews ("In this section, we present…"), section-closing recaps ("As we have seen…"), and inter-section stitching ("Having described X, we now…") removed, with at most one Introduction roadmap? <!-- policy:PROSE.FRACTAL_SUMMARY -->
...
- No semicolons in body paragraphs — two independent clauses joined by a semicolon split into two sentences (not re-punctuated to a comma)? Inline-math conditioning notation, `\;` thin spaces, and list-item separators are exempt. <!-- policy:PROSE.SEMICOLON_RESTRICTION -->
