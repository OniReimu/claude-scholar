...
- Sentence rhythm varies rather than sitting in one length band (stdev ≥10 words, 15–30-word band ≤55%, read as a diagnostic, not a target to merge sentences toward), every short sentence carries a checkable claim or an anchor rather than announcing one ("The difficulty is structural."), no sentence stacks two or more subordinate units (relative clause + insert + second predicate) where one-idea sentences would read cleaner, and no setup-then-punchline split delivers a rebuttal in three words ("One might expect X. It does not.")? <!-- policy:PROSE.RHYTHM_VARIANCE --> <!-- policy:PROSE.CLAUSE_CHAIN --> <!-- policy:PROSE.ANNOUNCEMENT_SENTENCE --> <!-- policy:PROSE.THEATRICAL_SPLIT -->
