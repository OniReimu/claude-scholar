...
- **2026-09-24 (v1.33.2)**: New rule `PROSE.CLAUSE_CHAIN`: a sentence carries one layer of subordination. A chain of a relative clause, an inserted phrase and a second predicate ("…, which tells … and, without labels, sets …") is split into one-idea sentences, each opened by a concrete subject or an anchor. These chains often have only two or three commas and stay under 35 words, so the comma and length rules miss them. `PROSE.RHYTHM_VARIANCE` now treats sentence-length variance as a diagnostic: an edit pass fixes orphan short sentences and clause chains and leaves the author's sentence lengths alone. Merging sentences to raise the variance is no longer allowed, because the merged sentences grew new clause chains and read as too long.
