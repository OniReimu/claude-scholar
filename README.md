...
- **2026-09-19 (v1.32.1)**: Skill validation runs in CI, which it had not been doing. `scripts/validate-skills.sh` existed but no workflow called it, so nothing checked anything on push — `.github/workflows/` held only `policy-validate.yml`. The replacement's first run found two things sitting in `main`. **`skills/lineage/SKILL.md` carried frontmatter that is not valid YAML**: its description contains `the symptom it exists for: 'what am I running right now'`, and an unquoted colon-space inside a plain scalar makes the parser read a nested mapping, so it was the only one of 53 that would not parse and any strict reader of it got nothing. And **a contributor's home directory was committed across three files** — `plugins/installed_plugins.json` and `known_marketplaces.json` are Claude Code's own plugin-manager state carrying `/Users/<name>` two dozen times and read by nothing here, now untracked and gitignored with the file left on disk, while `agents/paper-miner.md` hardcoded the same path as the destination for knowledge files, which was wrong for everyone else as well as identifying. Three skills also named themselves after a different skill (`agent-identifier` said "Agent Development", `command-name` said "Plugin Structure"); the check slugifies before comparing, so Title Case stays acceptable where it matches and the target is a name pointing at a different skill, not a naming style. The first draft of the checker reported **151 findings of which almost none were defects**, which forced two narrowings that are the substance of the change: only references into a skill's own `references/`, `scripts/`, `assets/` or `examples/` are resolved, because everything else a SKILL.md names is a path in the *user's* project — an output the skill tells the agent to create, a template holding `{slug}`, a shell command — and is not this repository's to resolve; and only tracked files are scanned for local paths and credentials, since the question is what the repository publishes, with conventional stand-ins (`/Users/name`) and documented examples (AWS's `AKIAIOSFODNN7EXAMPLE`) recognised. Each check was verified by injecting its fault and confirming the gate fails — and the first attempt at the broken-reference test passed *while the fault was present*, because `sed` had rewritten the link text rather than its target when both read the same, so the test was corrected rather than the green result trusted.
- **2026-09-19 (v1.32.0)**: `research-workspace-layout` — adopting the [research-workspace contract](https://github.com/DELONG-L/Research-Workflow-Skills) in a repository that already has years of accumulation, plus every way attaching Overleaf as a submodule goes wrong. The contract names a topology; this skill treats it as a **closed set**, and that is the whole difference: a layout that merely describes what exists cannot prevent the next `cache_v2_now_rx`, because **eleven parallel cache directories do not appear from anyone deciding to have eleven — they appear because nothing said where the twelfth would have to go**. Two same-name-opposite-meaning collisions are called out before any move: `paper/` is an independent repository to the contract and an ordinary directory to most monorepos, and `experiments/` is the record root to the contract and the code tree everywhere else — the tempting fix of nesting records deeper (`experiments/records/EXP-*`) invents a directory the contract does not have, which is the habit being cured. The trap underneath both is the **ignore policy**: a repository blocking `*/scripts/` by directory name keeps its generators out of version control, which makes `local.source_roots` (defined as version-controlled roots) and every figure's `code_revision` unimplementable, so that is step one and it ships with a two-direction dry-run — the forgotten direction being *which already-tracked files would the new rules now ignore*, where a sweeping rewrite typically strands thousands. Migration carries two load-bearing rules: **leave nothing at the old path and add no symlinks**, because with eleven similarly-named cache directories a missed reference that silently resolves to a *different* one can change a published number while a `FileNotFoundError` cannot; and **find self-locating path code before moving it**, since `PROJECT_ROOT = Path(__file__).resolve().parent.parent` lands silently wrong the moment a package descends into `src/` and takes every derived path with it. `tests/` goes to `src/tests/` and `docs/` is refused outright — not a missing category but an overflow pile whose contents are already `design.md`, `results.md`, `data-manifest.yml` and paper material. The bundled `scripts/validate_workspace.py` checks contract front matter, source roots that are declared but gitignored, `EXP-*` records outside their root, link files whose `parent_contract` resolves to nothing, manifest provenance, paper-only remotes on the primary repository, submodules committed as ordinary blobs, and root entries outside the closed set — and it accepts the four manifest forms the contract allows that a naive checker calls broken (a `generator` describing a manual process, semicolon or YAML-list `source_data`, `archive.zip::inner/path`, and `code_revision` declared once for the manifest with `data_snapshot` standing in for `experiment_id`). The Overleaf reference covers the failure modes in the order they bite: a **detached HEAD** reports one sha and hides that the parent is pinned commits behind the manuscript's real state; **double-tracking** (registered in `.gitmodules`, committed as `100644` blobs) makes `git submodule status` omit the submodule entirely, so the manuscript's history is invisible from the research side; `git add -A` converts every nested repository into a broken gitlink at once; a submodule's `.git` is a *file*, so local excludes belong in `.git/modules/<name>/info/exclude`; and the Overleaf token lives in `.git/config`, which is fine until someone copies it into `.gitmodules`. Only `paper/` is ever the submodule — Overleaf carries the manuscript, so analysis code moves *out* — and with several manuscript versions `paper/` becomes a container (`iclr27/`, `neurips26/`, `arxiv/`) with only the Overleaf one linked. It serves both entry points: a project started from scratch scaffolds from `assets/` (contract template, paper link file, figure manifest, the seven-file experiment record, and a gitignore starter that blocks by kind), and an existing project retrofits through the playbook — and the new-project path is not a smaller version of the retrofit, it is the case where the expensive problems never form: `experiments/` never becomes a code tree, `code_revision` is available from the first figure, and the first record is opened at design time rather than reconstructed from directory timestamps.
- **2026-09-14 (v1.31.0)**: `peer-review` — a routing skill for the one review Claude Scholar had no skill for: reviewing someone else's submission for a venue. All four existing review skills are author-side (`paper-self-review`, `claim-architecture-review`, `writing-anti-ai`, `review-response`), which makes the failure mode reaching for the nearest one and producing a submission checklist where a review was asked for. The reviewer-side workflow lives in [More Than Peer Review](https://github.com/DELONG-L/More-Than-Peer-Review-Skill), installed separately; this skill classifies the request and hands off **before the manuscript is opened**, passing the user's original request and the named file rather than a summary, so the external skill starts from the source and no candidate criticism or recommendation formed on this side crosses over. It carries no review logic at all, and says so in the one place that matters: the handoff names the external `SKILL.md` and references as authoritative **without enumerating their parts** — the previous draft listed "intake gate, review method, output rules, validation requirements", and the upstream update of the same week removed the intake gate, which is exactly how a thin router grows a second copy that drifts. Both harnesses share one clone symlinked into `~/.claude/skills/` and `~/.codex/skills/`, so `/more-than-peer-review` and `$more-than-peer-review` resolve to the same file and one `git pull` updates both; the earlier plan of two harness-specific routers is dropped, since after that consolidation the only thing that differed between them was the invocation form.
- **2026-09-11 (v1.30.0)**: `PROSE.CAUSAL_CONNECTIVE` gets the instrument its own argument requires. The card had already proved, with a 42-sentence blind eval, that per-instance judgment on `X, so Y` has **no** discriminating power — under the tightened criteria pre-GPT prose scores 64% "should fix" against a contemporary draft's 29%, the order reversing — and that the entire effect lives in **density**: 0.18–0.28 per 1000 words in the pre-GPT corpora against 3.76 in a measured local draft. The remedy gate was nonetheless per-instance and deliberately conservative (three diagnosable subclasses, everything else kept), and the only instrument was a report-only table counting five formal connectives, raw and unnormalised, **which never counted `, so` at all**. The one quantity with discriminating power was therefore the one quantity the executor could not see, and the failure this produces is the quiet kind: a draft at fifteen times baseline is scanned honestly, each hit is correctly judged keepable, and the pass reports nothing — repeatedly, until a human says "there are a lot of these". Lint now prints a second report-only line under the rule — `, so` and formal connectives as per-1000-word rates with their ratio, the pre-GPT baseline printed alongside — and both card and skill make the scan order explicit: **read the density first, then judge instance by instance**. The keep-bias is stated as what it always was, a calibration for baseline-density prose: at a ratio near 1:8–12 it holds and zero findings is the correct result; where the ratio collapses, the surplus over baseline is real and what should be reported is that whole-document fact, not the per-instance zero. No threshold and no target ship (`PROSE.SEMANTIC_IDLING`'s refusal of compression targets, for its reason: a published target is something to edit toward). A second accumulation path is named to match the one the rule already documented: punctuation-clearing passes push relations up into `therefore`, and **a "write it more plainly" instruction pushes them down into `so`** — which is a register drop on an edit, so it now has an earlier interception point as `CONNECTIVE-DOWNGRADE` in `PROSE.REGISTER_PRESERVATION`, judged on the diff at the moment of substitution rather than on whole-document density several passes later, when every instance has become locally defensible.
- **2026-09-01 (v1.28.0)**: The polish loop gets an exit that is a verdict instead of a count. `PAPER.REVISION_CLOSURE` makes self-review end with exactly one of four calls on the whole manuscript — **STOP_REVISING** (no substantive root cause large enough to reopen revision; remaining findings become directional advice or move to the submission-readiness axis, where they coexist with the decision to stop), **ONE_BOUNDED_ROUND** (one local substantive problem, and the verdict must name the root cause, fix the sections in scope, and cap it at a single round), **REOPEN_SUBSTANTIVE_REVISION** (a claim does not hold, experimental support has a gap, or the argument structure is broken — none of which rewriting wording can close), **UNASSESSED** (missing sections, placeholder text or truncated body; calling STOP_REVISING on half a manuscript is a fabricated verdict). The previous exit was *does everything pass?*, and this repository's own evidence says a count-based exit cannot hold: `PROSE.CAUSAL_CONNECTIVE` recorded connective monoculture accumulating ACROSS passes — no single rule failing on any single pass, every per-instance check green, 22 of 26 formal connectives ending up `therefore`. The consequence is that **the number of passes is itself a risk variable**: every repair round settles new statistical signature into the draft, and *stop at zero findings* prices that cost at zero. Per-rule guardrails cannot cap it — guardrails are per-instance, accumulation is cross-instance — so closure sits above every rule and arbitrates only whether another round runs at all, which is why its `conflicts_with` is empty. Three grounds for the verdict are banned outright: **finding counts** ("N violations remain" is lint's output, not a reading of the manuscript), **abstract perfection** ("it could be better" holds for every draft and therefore informs about none), and **acceptance-probability guesses** (predicting a quantity the card cannot observe). **No numeric threshold ships** — the same refusal `PROSE.SEMANTIC_IDLING` makes against embedding similarity and compression targets, for the same reason, with one aggravating factor: the cheapest way to lower a finding count is to make the rules harder to trigger. An anti-loop clause closes the obvious hole — **the same root cause may not trigger a bounded round twice**: if one scoped rewrite did not resolve it, it was never local, so it is either reopened on new evidence or kept on the manuscript as a stated limitation.
- **2026-08-28 (v1.27.0)**: Connective monoculture gets a structural fix, after recurring in two consecutive projects. The mechanism is systemic: the three punctuation-clearing rules (`EM_DASH_RESTRICTION`, `SEMICOLON_RESTRICTION`, `MIDSENTENCE_COLON`) all push the relation the punctuation carried — "so", "namely", "therefore" — into the lexical layer, and `CAUSAL_CONNECTIVE`'s menu then hands over its safest item: `therefore` is listed first, defined widest, and the card's own tell ("cannot say which kind = the causality was never thought through") never fires on it, because `therefore` can always be said. The existing guardrail ("do not batch-replace") guards a single pass; what actually happens is accumulation ACROSS passes — one or two per clearing pass, each locally defensible, 22 of 26 formal connectives ending up `therefore` with every per-instance check green. Three gaps closed. **A report-only connective-distribution table** now prints under `CAUSAL_CONNECTIVE` in lint (`therefore N · thus N · hence N · consequently N · accordingly N`) — deliberately not a violation and deliberately not a ratio threshold, since an all-entailment paper is legitimately therefore-heavy; the table exists so the one-line recheck — *is this really all the same kind of causality?* — runs against numbers instead of an impression. **The remedy ladder is reordered**: delete first (the causality is already carried by the previous sentence, or double-marked with `However`/`Because` — 8 of 10 measured fixes), then subordinate, then re-type by meaning; never pad `hence` for the distribution's sake. **The two punctuation cards that lacked it** now declare `conflicts_with: CAUSAL_CONNECTIVE` mutually (`SEMICOLON` already had it from v1.24) and carry the same instruction: after a clearing pass, re-measure the distribution.
- **2026-08-28 (v1.26.0)**: `PROSE.OVER_DEFENSIVE` gains a **sentence layer** — disclaimer-style negative predicates — because its five existing clauses are all position-and-count judgments, and a single well-placed *"We do not select an optimal policy"* passes every one of them while sixteen of them add up to a manuscript that spends its time filing disclaimers (measured: 1.7 per 1000 words). The card's own exclusion list was actively protecting the shape: disclaimers impersonate scope statements, and scope statements are exempt. Triage first, and only class A is in scope: **A** — author or authored-artifact subject + refusal predicate; **B** — factual negation (definitions, findings, numeric results), 17 of 33 on the measured manuscript, which a rule without this line would eat. The remedy is one move, not deletion: flip *"we do not do X"* into *"what we do is Y"* — `PROSE.SELF_UNDERMINING`'s third step landed at sentence level, information preserved and usually increased (*"policy selection remains with the venue"* names who selects). Three keep classes: a finding that is itself a negative proposition (flipping it is over-claiming), a reasoned methodological choice (flipping invents what was used), and the only place the excluded items are named. Mechanical flipping is forbidden — twelve measured rewrites used six different structures, and uniformity would trade the disclaimer fingerprint for a template one. A locator pattern ships (author subjects + refusal predicates), locate-only: the same shape carries A-class disclaimers, keep-class choices and B-class facts, and no subject list can split them. Wiring it exposed an engine defect: **lint ran patterns only when `check_kind: regex`**, so a judgment card carrying a regex locator parsed clean, validated clean, and never executed — the gate is now pattern-presence, which 9c already guarantees is intentional. Cross-fires with `PROSE.NEGATION_CONTRAST` on neither/nor and bound-subject sentences are annotated in the corpus as by-design locate-layer costs rather than suppressed.
