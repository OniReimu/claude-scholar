...
**基线已补，方向是反的**：定位 pattern 扫四个语料——arXiv 2019–21 **0.23**/千词，arXiv 2025–26 **0.09**，作者本人 pre-GPT 0.02、当前 draft 0.06。免责句在**老论文里更常见**（`we do not consider ...` 式 scope 声明是传统写法）。因此本层**明确不是 AI 痕迹**，是作者选定的风格约束；也因此三类保留与 B 类豁免不是装饰——真实人类散文里合法的否定构造密度更高，盲评实测 12 条真实定位句**零破坏性改写**（两条被翻的都完整保留了被点名项）。
