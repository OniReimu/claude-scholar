...
⚠️ **判据 2 的参照读者是冷进本节的人，不是刚读完上一节的人。** 审稿人跳着读：很多人从 `\section{Method}` 或 `\section{Experiments}` 直接进入，前文一个字没看。对这样的读者，"回翻成本"是无穷大，判据 2 **按构造成立**。所以顶层 `\section` 的开头段落若是该节**唯一的路线图**，应当保留——即便它与上一节尾部只隔两行。实测一次误删（2026-09-21）：Method 开头"先学 *how to construct*、再学 *when and how strongly*"那句被按"距 Intro 太近"删掉，而它正是冷进第三章的审稿人唯一能抓住的一句。

这条豁免**只放行承载内容的路线图**：它得说各部分**做什么**（"stage 2 fixes the memory and adapts the interfaces"），不是各部分**叫什么**（"Section 3.2 describes stage 2"）。后者仍是本卡要删的预告。斜体标出两三个承重短语是给跳读者的视觉锚点，不算 `PROSE.FORMATTING_RESTRAINT` 违规。同理，指向附录的一句指针不是主张，在节首和用到细节处各出现一次不构成复述。
