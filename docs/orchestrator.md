...
产物文件保持在项目原有位置（项目根目录、`analysis-output/` 等），run state 只存储相对路径、可选的 `tracked_files` 元数据和 hash 指纹。
...
| 7 | `writeup` | 论文撰写 | 否 |
| 8 | `architecture_review` | claim/段落架构审计（减法，propose-only：paragraph-audit + relocation-map） | 否 |
| 9 | `self_review` | 自审 + policy lint | 否 |
| 10 | `rewrite` | 应用结构方案 / 修复违规（shared apply stage） | 是 |
| 11 | `rebuttal` | 审稿回复 | 是 |
| 12 | `post_acceptance` | 录用后处理 | 是 |
...
v1.4.2 起，指纹收集不再完全依赖 agent 自行选择文件：

1. stage contract 中 `expected_artifacts.kind = file` 的文件会自动纳入追踪集合
2. `writeup` 阶段会从 `artifacts.writeup.main_tex` 自动展开本地 LaTeX 依赖：
   - `\input{...}` / `\include{...}`
   - `\bibliography{...}` / `\addbibresource{...}`
   - `\includegraphics{...}`
3. `extraPaths` 仅用于补充小而关键的文件，不能替代 contract baseline
4. 不递归 hash 整个 figures 目录，也不追踪大体积原始数据目录

推荐调用：

```javascript
const { tracked_files, fingerprints } = orch.fingerprintStageArtifacts({
  cwd,
  run,
  stageId: 'writeup',
  extraPaths: ['main.tex']
...
Polish 模式用于对已有 draft 进行风格和规则合规性改写，跳过从零开始的 stage 1-6。

### 触发方式

用户使用以下任一说法时触发 polish 模式：

- "帮我 polish 这篇论文"
- "改写/重写这个 draft"
- "这篇 draft 需要按我们的风格重写"
- "Polish this paper"
- "Rewrite this draft to match our style"

LLM 识别意图后调用 `initPolishRun()` 而非 `initRun()`。

### Polish 流程

```
intake (polish mode, 自动 done)
...
1. 读取 `self_review` 阶段的 `violation_report`
...
### Session End (`hooks/session-summary.js`)

会话结束日志现在包含：
- 活跃 run 的 stage/status 摘要
- 当前阶段的 tracked file 数量（如有）
- 最近几条 orchestrator 事件
