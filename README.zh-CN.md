...
- **2026-09-19 (v1.32.1)**: skill 校验终于在 CI 里跑起来了——此前并没有。`scripts/validate-skills.sh` 一直存在，但没有任何 workflow 调用它，`.github/workflows/` 里只有 `policy-validate.yml`，所以 push 时什么都没被检查。替换版第一次跑就在 `main` 里抓到两样东西。**`skills/lineage/SKILL.md` 的 frontmatter 不是合法 YAML**：description 里含 `the symptom it exists for: 'what am I running right now'`，纯量里未加引号的冒号加空格会被解析器当成嵌套 mapping，它是 53 个里唯一解析不了的，任何严格读取该 frontmatter 的工具从它那里什么也拿不到。以及**一位贡献者的 home 目录被提交在三个文件里**——`plugins/installed_plugins.json` 与 `known_marketplaces.json` 是 Claude Code 插件管理器自己的状态文件，含 `/Users/<name>` 二十余处且仓库里无人读取，现已撤出版本控制并加入 gitignore、文件留在盘上；`agents/paper-miner.md` 则把同一路径硬编为知识文件的写入目标，那对其他任何人都是错的，不只是身份信息。另有三个 skill 用了指向别的 skill 的名字（`agent-identifier` 写着 "Agent Development"、`command-name` 写着 "Plugin Structure"）；检查先 slug 化再比较，所以 Title Case 只要能对上仍然合法——要抓的是"名字指向另一个 skill"，不是命名风格。检查器初版报了 **151 条，其中几乎没有真缺陷**，由此逼出的两处收窄才是这次改动的实质：只解析指向 skill 自身 `references/`、`scripts/`、`assets/`、`examples/` 的引用，因为 SKILL.md 提到的其余路径都在**用户的**项目里——skill 让 agent 创建的产物、含 `{slug}` 的模板、shell 命令——不归本仓库解析；以及只扫 git 跟踪的文件来查本机路径与凭据，因为要问的是"仓库发布了什么"，同时放行 `/Users/name` 这类约定占位与 AWS 文档示例 `AKIAIOSFODNN7EXAMPLE`。每条检查都通过注入对应故障确认能挡下——而断链那条第一次跑时**故障就在文件里却是绿的**，原因是链接文本与目标同名，`sed` 改掉的是文本不是目标，于是修正了测试而不是采信那个绿色。
- **2026-09-19 (v1.32.0)**: `research-workspace-layout` — 把 [research-workspace 契约](https://github.com/DELONG-L/Research-Workflow-Skills) 落地到一个已经积累多年的仓库，外加把 Overleaf 挂成 submodule 的全部踩坑。契约给出一套拓扑；本 skill 把它当作**闭集**，差别全在这里：只描述现状的布局挡不住下一个 `cache_v2_now_rx`——**会长出十一个并列的 cache 目录，不是因为有人决定要十一个，而是没有任何地方规定第十二个必须去哪**。动手前先点破两处同名反义：`paper/` 在契约里指独立仓库、在多数 monorepo 里只是普通目录；`experiments/` 在契约里是记录根、在别处是代码树——而"把记录再往下塞一层"（`experiments/records/EXP-*`）恰恰是在发明契约没有的目录，正是要治的那个习惯。两者底下还压着 **ignore 策略**这个陷阱：按目录名挡掉 `*/scripts/` 的仓库，等于把生成器排除在版本控制之外，于是 `local.source_roots`（定义就是"受版本控制的根"）与每张图的 `code_revision` 都无法实现，所以它是第一步，并且配了双向 dry-run——被忘掉的那个方向是**新规则会把哪些已 tracked 的文件划进 ignore**，大刀阔斧的重写通常在这里搁浅数千个文件。迁移有两条压舱规则：**旧路径不留任何东西、也不建 symlink**，因为十一个相似的 cache 目录并存时，漏改的引用若**静默解析到另一个**就可能改掉已发表的数字，而 `FileNotFoundError` 不会；以及**搬之前先找出靠自身位置算路径的代码**，`PROJECT_ROOT = Path(__file__).resolve().parent.parent` 在包下沉进 `src/` 的那一刻会静默指错，并把所有派生路径一起带偏。`tests/` 归 `src/tests/`，`docs/` 直接拒绝——它不是缺失的类别而是溢出堆，里面的东西本来就该是 `design.md`、`results.md`、`data-manifest.yml` 和论文材料。附带的 `scripts/validate_workspace.py` 检查契约 front matter、声明了却被 gitignore 的 source root、越出记录根的 `EXP-*`、`parent_contract` 解析不到的 link file、manifest 溯源完整性、主仓库上的论文专用远端、被当成普通 blob 提交的 submodule，以及根目录的越界项——并且接受契约允许、朴素检查器却会误报为坏掉的四种 manifest 写法（描述人工流程的 `generator`、分号或 YAML 列表形式的 `source_data`、`archive.zip::内部路径`、以及在 manifest 顶层声明一次的 `code_revision` 配合用 `data_snapshot` 顶替 `experiment_id`）。Overleaf 那份 reference 按"咬人的先后"排列失效模式：**detached HEAD** 只报一个 sha，掩盖父仓库被钉在手稿真实状态之前多少个提交；**双重跟踪**（登记在 `.gitmodules` 却以 `100644` blob 提交）会让 `git submodule status` 干脆不列它，手稿历史从研究仓库侧完全不可见；`git add -A` 一次就能把每个嵌套仓库都变成坏 gitlink；submodule 的 `.git` 是**文件**，本地排除要写进 `.git/modules/<name>/info/exclude`；Overleaf 令牌住在 `.git/config` 里，本身没问题，除非有人把它抄进 `.gitmodules`。永远只有 `paper/` 是 submodule——Overleaf 只 care 手稿，所以分析代码要搬**出去**——多版本手稿时 `paper/` 变成容器（`iclr27/`、`neurips26/`、`arxiv/`），其中只有 Overleaf 那个被链接。 两种入口都覆盖：全新项目从 `assets/` 脚手架起步（契约模板、paper link file、figure manifest、EXP 七件套、按 kind 挡的 gitignore starter），已有项目走 playbook retrofit——而全新项目那条路不是 retrofit 的缩小版，它是那些昂贵问题根本不会形成的情形：`experiments/` 不会变成代码树、第一张图就有 `code_revision` 可写、第一条实验记录是在设计时打开而不是事后从目录时间戳重建的。
- **2026-09-14 (v1.31.0)**: `peer-review` —— 补上 Claude Scholar 唯一没有 skill 的那种 review：**替某个 venue 审别人的稿子**。既有四条 review 线全是作者侧（`paper-self-review` / `claim-architecture-review` / `writing-anti-ai` / `review-response`），于是失效形态是抓起最近的那条，在别人要一份 review 的时候交出一份投稿自检清单。审稿侧的工作流在 [More Than Peer Review](https://github.com/DELONG-L/More-Than-Peer-Review-Skill)，单独安装；本 skill 只做请求分类与交接，且**在打开稿件之前**就交出去——递过去的是用户的原始请求和文件名而不是摘要，让外部 skill 从原材料开始，不让这一侧形成的任何候选批评或推荐倾向越界。它自身零条评审逻辑，并在最要紧的那处把这件事写死：交接条款声明外部 `SKILL.md` 与 references 具有权威性，**但不枚举它们的组成部分**——上一版枚举了「intake gate、review method、output rules、validation requirements」，而上游同周的更新恰好删掉了 intake gate，薄路由长出会漂移的第二份副本就是这么开始的。两个 harness 共用一个 clone，分别符号链接进 `~/.claude/skills/` 与 `~/.codex/skills/`，`/more-than-peer-review` 与 `$more-than-peer-review` 指向同一份文件，一条 `git pull` 同时更新两边；原先「两个 harness 各一份路由」的方案作废——共用之后两者的唯一差异只剩调用形式。
- **2026-09-11 (v1.30.0)**: `PROSE.CAUSAL_CONNECTIVE` 补上它自己的论证所要求的那把尺。卡片早已用 42 条真实句子的盲评证明：对 `X, so Y` 的**逐条**裁决判别力为零——收紧判据后 pre-GPT 散文 64% 判「应改」，当代 draft 只有 29%，顺序反转——全部收益来自**密度**：pre-GPT 语料 0.18–0.28/千词，本地 draft 实测 3.76/千词。但改写闸门仍然是逐条的、而且刻意保守（三个可诊断子类，其余一律保留），而唯一的仪器是一张 report-only 表，只数五个正式连接词、不归一化、**根本不数 `, so`**。于是唯一有判别力的量，恰好落在执行者看不见的地方；由此产生的失效是安静的那种：一份十五倍基线的稿子被诚实地扫描，每一处都被正确判为该保留，整遍报告没问题——反复如此，直到人开口说「这种句式好像很多」。lint 现在在本卡下打印第二行 report-only 数据：`, so` 与正式连接词各自的每千词率、两者比值，pre-GPT 基线并排印出；卡片与 skill 同时把扫描顺序写死——**先读密度，再逐条判**。保留偏置被还原成它本来的身份：一个按基线密度做的校准。比值在 1:8–12 附近它成立，报零 finding 就是正确结果；比值塌掉时，超出基线的那部分是真的，此时该报告的是这个全文事实，而不是逐条裁决后的零。**不设阈值也不设目标值**（同 `PROSE.SEMANTIC_IDLING` 拒绝压缩率目标的理由：公布的目标会变成被凑的目标）。另外补上与既有那条对称的第二条累积路径：标点清理把关系往上赶进 `therefore`，而**「写得简单直白一点」把它们往下压成 `so`**——后者是编辑动作上的语域下降，因此现在有了更早的拦截点，作为 `PROSE.REGISTER_PRESERVATION` 的 `CONNECTIVE-DOWNGRADE` 在替换当场按 diff 判定，而不是等几个 pass 之后按全文密度发现，那时每一处单看都已经合理了。
