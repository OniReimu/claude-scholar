...
| 4. Paper Writing | `ml-paper-writing` skill + `paper-miner` agent; figures: `paper-figure-generator` (default conceptual) / `fireworks-tech-graph` (formal diagrams) / `scientific-figure-making` (data plots) | - |
...
- **research-workspace-layout**: 研究仓库目录契约的落地，**两种入口**——全新项目从 `assets/` 脚手架起步（契约模板 + link file + figure-manifest + EXP 七件套 + gitignore starter），已有老结构的项目走 `references/migration-playbook.md` retrofit。把 [research-workspace-governance](https://github.com/DELONG-L/Research-Workflow-Skills) 的 `PROJECT-WORKSPACE.md` 契约应用到已有仓库，外加把 Overleaf 作为 submodule 挂载的全部注意事项。拓扑按**闭集**处理（根目录只许 `experiments/ src/ configs/ scripts/ data/ artifacts/ paper/`）—— 会长出十一个 `cache_*` 不是因为有人决定要十一个，而是没有任何地方规定第十二个该去哪。自带 `scripts/validate_workspace.py`（契约 front matter、声明的 source root 是否真在版本控制里、`EXP-*` 记录是否越界、link file 的 `parent_contract` 是否解析得到、manifest 溯源完整性、submodule 是否被当成普通 blob 双重跟踪、根目录越界项）。**路由规则**: 目录整理 / 加契约 / 接 Overleaf submodule / "submodule has untracked changes" 一律走本 skill；`latex-conference-template-organizer`（模板整理）、`lineage`（实验谱系叙事）、`git-workflow`（通用 git 规范）都不能替代
